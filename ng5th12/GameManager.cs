...
public class GameManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Các thao tác khởi tạo khác cho GameManager
        float gameTime = _clock.GetElapsedTime();  // Lấy thời gian đã trôi qua
        Debug.Log("Elapsed Time: " + gameTime + " seconds");
    }
    ...
    */
    ...
    public void RestartGame()
    {
        ...
        {
            Debug.LogError("_livesText is not assigned!");
        }
    ...
    public void PauseGame()
    {
        ...
        if (ispause == true)
        {
            ...
            SceneManager.LoadScene("Pause");  // Chuyển sang scene Pause
        }
        else
        {
            Time.timeScale = -1;
    ...
    public void NewGame()
    {
        ...
        // Reset thời gian về 0 bằng cách gọi OnStartGame() từ Clock
        _clock.OnStartGame();  // Gọi phương thức OnStartGame để reset thời gian

        // Reset Level và Difficulty
        PlayerPrefs.SetInt("Level", 1);  // Set lại level về 1
        PlayerPrefs.SetString("GameMode", "EASY");  // Chế độ mặc định là EASY

        // Tạo game mới (bảng mới, các giá trị cần thiết)
        CreateNewGame(Generator.DifficultyLevel.EASY);

        // Load lại scene DoGame
        SceneManager.LoadScene("DoGame");
    }
}